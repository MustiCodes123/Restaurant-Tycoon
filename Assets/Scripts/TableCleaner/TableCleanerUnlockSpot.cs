...
/// <summary>
/// The unlock spot where player stands to pay and unlock a table cleaner.
/// Similar to JanitorUnlockSpot.
/// </summary>
public class TableCleanerUnlockSpot : MonoBehaviour
{
    [Header("References")]
    ...
    [SerializeField] private TextMeshProUGUI cleanerNameText;
    ...
    private TableCleanerUnlock tableCleanerUnlock;
    ...
    private void Update()
    {
        if (!isPlayerInRange || !isPaymentActive || tableCleanerUnlock == null) return;
    ...
    {
        if (tableCleanerUnlock == null || CurrencyManager.Instance == null) return;
    ...
    public void Show(TableCleanerUnlock unlock)
    {
        ...
        // Update cleaner name if we have the text component
        if (cleanerNameText != null && unlock.UnlockData != null)
        {
            cleanerNameText.text = unlock.UnlockData.CleanerName;
        }
        ...
        {
            string cleanerId = unlock.UnlockData.CleanerName.Replace(" ", "_");
            DynamicMissionManager.Instance.RegisterTableCleanerUnlockMission(cleanerId, unlock.UnlockData.CleanerName);
        }
        
        // Start pulse animation
        StartPulseAnimation();
        
        Debug.Log($"[TableCleanerUnlockSpot] Show called - TableCleanerUnlock set: {tableCleanerUnlock != null}, isPlayerInRange: {isPlayerInRange}, playerTransform: {playerTransform != null}");
    }
    ...
    public void Hide()
    {
        ...
        if (DynamicMissionManager.Instance != null && tableCleanerUnlock != null && tableCleanerUnlock.UnlockData != null && !tableCleanerUnlock.IsUnlocked)
        {
            string cleanerId = tableCleanerUnlock.UnlockData.CleanerName.Replace(" ", "_");
            DynamicMissionManager.Instance.RemoveTableCleanerUnlockMission(cleanerId);
        }
    ...
    private void CompletePayment()
    {
        ...
        if (DynamicMissionManager.Instance != null && tableCleanerUnlock != null && tableCleanerUnlock.UnlockData != null)
        {
            string cleanerId = tableCleanerUnlock.UnlockData.CleanerName.Replace(" ", "_");
            DynamicMissionManager.Instance.CompleteTableCleanerUnlockMission(cleanerId);
        }
    ...
    {
        if (isPaymentActive || tableCleanerUnlock == null) return;
        ...
        Debug.Log("[TableCleanerUnlockSpot] Payment started!");
    }
    ...
    public void CancelPayment()
    {
        ...
        Debug.Log("[TableCleanerUnlockSpot] Payment cancelled!");
    }
    ...
    private void CheckForPlayerOverlap()
    {
        ...
        foreach (Collider col in overlappingColliders)
        {
            if (((1 << col.gameObject.layer) & playerLayer) != 0)
            {
                ...
                StopPulseAnimation();
                Debug.Log($"[TableCleanerUnlockSpot] Player already overlapping on Show!");
                return;
            }
        }
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        Debug.Log($"[TableCleanerUnlockSpot] Player entered - CurrentMoney: {(CurrencyManager.Instance != null ? CurrencyManager.Instance.CurrentMoney.ToString() : "CurrencyManager null")}, TableCleanerUnlock null: {tableCleanerUnlock == null}");
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        Debug.Log($"[TableCleanerUnlockSpot] Player exited - Was payment active: {isPaymentActive}");
